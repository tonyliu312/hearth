...
- **Your infrastructure** — the switch, the NAS, the gateway router: temperatures, fans, redundant PSUs, per-disk health. The boxes that carry the cluster but don't compute, and take it down with them when they overheat
...
| **Infrastructure** — MikroTik/RouterOS · Synology DSM (SNMP) · OpenWrt (node-exporter-lua) | ✅ Read-only | Chassis & chip temps · fan RPM · redundant PSU state/W/V · per-disk temp & status · uptime | 🟢 Drop-in · see [`docs/topology.md`](docs/topology.md) `infra:` |
