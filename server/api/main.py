...
def _win_ratio(key: str, num, den, now: float, win: float = None, min_age: float = None):
    """(Δ分子/Δ分母, 窗口秒)。【引擎速度】口径专用。

    ⚠️ 分母是【逐请求忙碌秒之和】, 不是墙钟秒: N 个请求并发时每墙钟秒累积 N 秒。
       所以这个比值读作【一个请求流有多快】, 而墙钟负载读作【这台机器总共产出多少】,
       两者在并发下必然分叉 —— 不写清楚会被当成"哪个算错了"。
    ⛔ 但"聚合吞吐 / 引擎速度 = 平均并发数"【不是恒等式】, 只是同一数量级的粗略指示,
       不能当公式用。三组实测(2026-09-19, llama.cpp, 比值 vs requests_processing 的
       1Hz 采样均值): 5.53 vs 3(末值)、3.95 vs 2.93(本机 22:16, 17.0s 窗口:
       Δtokens=3,360 / Δ忙碌秒=67.0 → 引擎 50.1、聚合 198.1)、2.29 vs 2.75。
       比值落在 gauge 两侧都出现过。成因是两个方向相反的偏差同时存在:
       requests_processing 含预填与排队态(抬高 gauge), 而 tokens_predicted_seconds_total
       按请求累计的时间未必是纯解码(抬高比值)。
