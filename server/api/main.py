...
#     （obs 既有缺陷：bridge 容器抓不到 Atlas 本机 :9100，gpu-host node job down）
#
...
NODES = [
    {"id": "atlas", "name": "Atlas", "ip": "10.0.0.10", "class": "RTX 4090 host",
     "role": "Gateway · Edge", "obs_node": "gpu-host", "node_source": "direct",
    ...
    {"id": "spark-01", "name": "Spark-01", "ip": "10.0.0.21", "class": "DGX Spark",
     "role": "Inference · ray-head", "obs_node": "spark-a", "node_source": "obs",
    ...
    {"id": "spark-02", "name": "Spark-02", "ip": "10.0.0.22", "class": "DGX Spark",
     "role": "Inference · ray-worker", "obs_node": "spark-b", "node_source": "obs",
     "gpu": {"name": "GB10 Grace-Blackwell", "mem": 128, "fp16": 250.0, "fp4": 1000.0},
     "cpu": {"model": "Grace 20-core ARM", "cores": 20, "threads": 20},
     "ram": 128, "disk": 4096, "net": "200 GbE ConnectX-7",
     "services": ["vllm", "node-exporter", "dcgm"]},
    {"id": "spark-03", "name": "Spark-03", "ip": "10.0.0.23", "class": "DGX Spark",
     "role": "Inference · ray-worker", "obs_node": "spark-c", "node_source": "obs",
     "gpu": {"name": "GB10 Grace-Blackwell", "mem": 128, "fp16": 250.0, "fp4": 1000.0},
     "cpu": {"model": "Grace 20-core ARM", "cores": 20, "threads": 20},
     "ram": 128, "disk": 4096, "net": "200 GbE ConnectX-7",
     "services": ["vllm"]},
    {"id": "spark-04", "name": "Spark-04", "ip": "10.0.0.24", "class": "DGX Spark",
     "role": "Inference · ray-worker", "obs_node": "spark-d", "node_source": "obs",
     "gpu": {"name": "GB10 Grace-Blackwell", "mem": 128, "fp16": 250.0, "fp4": 1000.0},
     "cpu": {"model": "Grace 20-core ARM", "cores": 20, "threads": 20},
     "ram": 128, "disk": 4096, "net": "200 GbE ConnectX-7",
     "services": ["vllm"]},
...
app = FastAPI(title="Hearth · Home AI Compute Monitor API", version="1.0.0")
...
async def _obs_node_live() -> dict[str, dict]:
    ...
    for obs_node in universe:
        ...
        # "显存"应取统一内存占用（模型实际驻留处）。仅 RTX4090(gpu-host)
        # 是独显，用 DCGM FB。
        is_unified = obs_node != "gpu-host"
...
async def _node_payload() -> list[dict]:
    ...
    # Atlas 的 GPU 走 obs DCGM（obs_node=gpu-host），node/hwmon 走直采
    atlas_gpu = obs_live.get("gpu-host", {})
    out = []
    for n in NODES:
