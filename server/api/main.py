...
def _win_ratio(key: str, num, den, now: float, win: float = None, min_age: float = None):
    ...
    ⚠️ 分母是【逐请求忙碌秒之和】, 不是墙钟秒: N 个请求并发时每墙钟秒累积 N 秒。
       ...
       两者在并发下必然分叉, 比值约等于平均并发流数 —— 不写清楚会被当成"哪个算错了"。
       实测(2026-09-19 22:16, llama.cpp, 17 秒窗口): Δtokens=3,360 / Δ忙碌秒=67.0
       → 引擎速度 50.1 tok/s, 聚合吞吐 3,360/17 = 198.1 tok/s, 比值 3.95。
       (同期 requests_processing 的 1Hz 采样均值 2.93 —— 那个 gauge 还含预填/排队态
        且采样更粗, 所以只能数量级对齐, 不是严格相等。)
