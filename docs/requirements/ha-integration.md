...
| 当前 IP | `10.0.0.228`(曾 `.100`→`.71`→`.228`,别认死 IP) |
...
| `atlas` | .20 | **无** | GPU-HOST 不在 cuco 插座上 → 无墙功率(诚实降级) |
...
### 5.1 部署 exporter(Atlas 10.0.0.10)

```bash
cd /home/user/dev/AI-Monitor/server/integration
# 复用 ai-monitor-api 的 venv(已含 requests),仅补 prometheus_client
/home/user/dev/AI-Monitor/server/runtime/venv/bin/pip install \
  -i https://pypi.tuna.tsinghua.edu.cn/simple prometheus_client

# 冒烟:用 mDNS 抗漂移
HA_URL=http://homeassistant.local:8123 \
HA_TOKEN=$(cat ~/.config/ha/token) \
  /home/user/dev/AI-Monitor/server/runtime/venv/bin/python ha-exporter.py &
...
Type=simple
User=tony
WorkingDirectory=/home/user/dev/AI-Monitor/server/integration
...
ExecStart=/home/user/dev/AI-Monitor/server/runtime/venv/bin/python ha-exporter.py
Restart=on-failure
...
2. **断电前强制安全关机** —— `docker stop -t 20 vllm_node` → `ssh user@<ip> 'sudo -n shutdown -h now'` → ping 确认下电 → 才 `turn_off`。唯一例外:SSH 登不上(机器卡死)。
...
exporter 自带 `node="spark-01"` 等标签,与 Prometheus scrape 后聚合一致。但 hearth.yaml `nodes[].sources.obs_node_label` 是该节点在 obs Prometheus 里的真实标签(如 `spark-b`)。**Phase 6 spec 必须规定**:exporter 标签值 = `nodes[].id`(Hearth 内部 id);PromQL recording rule 在 `join` 时用 `on(node)` 还是 `on(obs_node_label)` 由 spec 决定。当前推荐:exporter 用 `nodes[].id` 作 `node` 标签值,与 Hearth 内部命名一致;join 时 `on(node)`。
...
PoC 在 `prometheus.yml` 硬码 `targets: [10.0.0.10:9105]`。Atlas IP 不会漂(它是网关本机);但开源版用户若不在 Atlas 跑,需文档化。**Phase 6 spec 决定**:
