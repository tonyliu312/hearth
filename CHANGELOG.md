...
- **Infrastructure section — the boxes that carry the cluster but don't compute.** GPU and node metrics say nothing about the fabric switch, the NAS, or the gateway router, yet each is a single point whose thermal failure takes everything with it: a switch that shuts down drops every node's management plane *and* its east-west fabric at once, and a NAS disk drifting toward its vendor shutdown threshold can halt the array without a single GPU metric moving. Declare the devices under `infra:` and Hearth renders chassis and chip temperatures, fan RPM, redundant-PSU state/watts/volts, and per-disk temperature and status. Three read paths, all read-only, all through *your* Prometheus — Hearth never talks to the devices directly: `mikrotik` reads RouterOS's generic `mtxrHealth` table (mirrors `/system/health/print` row for row, so another model's extra sensors appear without a schema change), `synology` reads DSM's system and disk MIBs, and `openwrt` skips SNMP entirely (routers rarely ship `snmpd`) to scrape `prometheus-node-exporter-lua` plus a six-line thermal collector whose metric name matches upstream node_exporter's `hwmon` collector, so one query covers routers and x86 hosts alike. Two temperature scales, because disks are far more fragile than silicon: 48/55 °C for disks against 70/80 °C for chips. Normal readings render in plain ink — colour is reserved for a reading that has actually crossed a line, and an all-healthy device shows no status pills, because a wall of green makes a real warning *harder* to spot. Omit the `infra:` block and the section renders nothing; it never invents devices. Docs: [`docs/topology.md`](docs/topology.md) `infra:` section, with ready-made scrape jobs and a reusable [`server/prometheus/snmp.yml`](server/prometheus/snmp.yml).

- **Distributed-training observability — new Training section** — Hearth was inference-only; a training run was invisible even while it saturated the cluster. Training asks different questions than serving ("is it still progressing, efficiently, without diverging or hanging?"), and raw GPU utilization actively lies during the classic hung-collective failure, where every GPU reads 100% util while the step counter is frozen. Two layers ship together. **(1) Substrate, zero-config** — built entirely from metrics your obs stack already scrapes (DCGM L1 + node_exporter, no profiling enabled, nothing installed on the training hosts): per-rank GPU, unified-memory headroom (the OOM-critical number on unified-memory SoCs), RoCE/InfiniBand collective bandwidth, straggler skew across ranks, ECC/Xid faults, and a silent-stall heuristic (high util + collapsed interconnect traffic + power well below the active median). **(2) Training signal, pluggable multi-source** — because open-source users won't share one framework, the signal layer is an adapter registry rather than a hardcoded reader: `TRAIN_SOURCE=auto` probes a metrics **JSON** file, a **Prometheus** textfile/endpoint, then **TensorBoard** `*.tfevents` (parsed by a dependency-free TFRecord/protobuf reader in `server/api/tfevents.py` — no tbparse, no tensorboard, nothing added to the trainer). All three normalize into one schema (`step / totalSteps / progress / epoch / loss / lossParts / gradNorm / lr / acceptance / meanAccept / perStepSec / etaSec / staleSec`) through an alias table, so field-name drift between frameworks is a config concern, not a code change. Missing fields are derived where honest (per-step time from accumulated `(step, ts)` pairs, total steps back-solved from a progress sample and cached, ETA from both) and left `null` where not. All reads are read-only over SSH or a local path — zero impact on the run. Absent a signal source the section degrades to substrate-only with an explicit "awaiting" note rather than fabricated curves. Docs: [`docs/training-observability.md`](docs/training-observability.md).

- **Concurrency visible in the Models section** — every backend that exposes a serving runtime (vLLM, llama.cpp, SGLang) already publishes `num_requests_running` / `num_requests_waiting`; hearth-api has been plumbing them into `/api/models` `live.running` / `live.waiting` for a while, but the UI silently dropped both fields, so operators reading "94 t/s" had no way to tell if that was 1 request at 94 t/s or 4 requests at 24 t/s each — a 4× resource-utilization difference that changes every capacity decision. The Models row now shows `N running` next to the tok/s bigmetric (` · M queued` painted warn-yellow when the gateway is queued), and the model detail view adds a full `DetailMetric` bar so the trend renders alongside TTFT/TPOT/KV. `data.js` learns to write `running` / `waiting` into the per-model live buffer from SSE payloads; existing fields untouched. No backend changes — the data has always been there.
...
- **Data-driven sections could render permanently invisible.** The scroll-reveal effect collected `.reveal` elements once on mount, but a section that returns `null` until its data arrives (Training, Infrastructure) isn't in the DOM at that moment — it mounts a second or so later when the first SSE frame lands, by which point nothing is observing it, so it stayed at `opacity: 0` forever. Every layer looked healthy in isolation (the API returned data, the SSE payload carried it, the component rendered) while the section was simply invisible on the page. The reveal effect now also runs a `MutationObserver` that registers `.reveal` nodes appearing after mount.
...
- **`ha.controller` — the Layer-2 rack-AC controller — is gone entirely.** `server/integration/ha-controller.py`, its systemd unit, both watchdog scripts, the installer, the config schema and the topology docs have all been deleted. Hearth reads Home Assistant for wall power and rack environment; it does not write back to it. Actuating a physical device — deciding when a compressor runs — is a control-plane problem with its own safety envelope, failure modes and blast radius, and it does not belong in an observability tool that ships to strangers. Operators who want load-aware cooling should drive it from a purpose-built controller with a fail-to-cool actuator, not from a dashboard. **Do not add it back.**
...
- **Node `kind` abstraction** — every node declares `kind: discrete | unified-arm-soc | apple-silicon`. Hearth picks the right VRAM% source per kind (DCGM FB for discrete, MemAvailable for unified). Replaces the legacy hard-coded `obs_node != "gpu-host"` check.
...
- **Atlas-specific code paths generalized.** What was `if n["id"] == "atlas"` is now `if n.get("node_source") == "direct"` — any node configured with a direct (non-Prometheus) scrape path qualifies, not just one specific host. What was `obs_live.get("gpu-host")` is now driven by the first `kind: discrete` node found in config.
- App title: `"Hearth · Home AI Compute Monitor API"` → `"Hearth API"` (v0.1.0).
